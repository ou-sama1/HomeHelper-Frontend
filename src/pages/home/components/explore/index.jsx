    ...
    >
      <ul className="absolute -top-0 flex w-full -translate-y-1/2 flex-row justify-between px-1 sm:px-5 md:px-10 lg:-top-7 lg:translate-y-0">
        {cards.map(({ id, icon, text }) => (
          <li
            className="flex h-32 w-32 flex-col items-center gap-4 rounded-full bg-tertiary px-5 py-4 shadow-lg md:h-40 md:w-40 lg:h-28 lg:w-1/4 lg:rounded-lg"
            key={id}
          >
            {icon}
            <span className="text-sm md:text-lg lg:text-xl">{text}</span>
          </li>
        ))}
      </ul>

      <h2 className="w-auto text-5xl font-bold lg:text-6xl xl:w-4/5">
        Explorer notre gamme complète de services professionnels
      </h2>
      <ul className="grid-col-1 m-0 grid gap-20 p-0 sm:grid-cols-2 sm:gap-10 lg:grid-cols-3 lg:gap-20 xl:gap-28">
        {servicesShowcaseData.map(({ id, name, description, icon }) => (
          <li
            key={id}
            className="flex w-96 flex-col items-center gap-5 text-center sm:w-72 xl:w-80"
          >
            {icon}
            <h5 className="text-xl font-bold md:text-2xl">{name}</h5>
